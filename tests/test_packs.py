...
from ordin.analyzers import analyze_tokens, analyzer_pack_bindings
from ordin.cli import main
from ordin.data import data_health, find_command, load_risk_rules
from ordin.packs import (
    discover_packs,
    enabled_packs,
    pack_list_payload,
)
from ordin.risk import check_command
from ordin.schema import resource_parity_errors, validate_named_schema
...
def test_default_packs_are_discovered_loaded_and_schema_valid():
    ...
    assert payload["schema_version"] == "ordin.pack_list.v1"
...
def test_selective_pack_loading_is_exact(monkeypatch):
    monkeypatch.setenv("ORDIN_PACKS", "git")
...
def test_unknown_configured_pack_is_reported_by_doctor(monkeypatch):
    monkeypatch.setenv("ORDIN_PACKS", "git,does-not-exist")
    health = data_health()
