...
def test_chmod_analyzer_tracks_mode_and_recursive_target():
    ...
    assert "filesystem.recursive_permission_change" in effects("chmod --recursive 777 ./shared")
...
def test_chown_analyzer_tracks_owner_and_target():
    ...
    assert "filesystem.ownership_change" in {item.effect for item in result.evidence}
    assert "filesystem.recursive_ownership_change" in {item.effect for item in result.evidence}
...
def test_git_force_push_emits_remote_write_and_history_rewrite():
    ...
    assert {"git.remote_write", "git.history_rewrite"} <= {item.effect for item in result.evidence}


def test_curl_upload_and_file_output_are_distinguished():
    assert "network.upload" in effects("curl --data-binary @payload.json https://example.com/api")
...
def test_pip_analyzer_normalizes_python_module_invocation():
    ...
    assert {"package.install", "code.execute"} <= {item.effect for item in result.evidence}
